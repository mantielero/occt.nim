...
## ! aManipulator->Detach();
## ! @endcode
## ! The last method erases manipulator object. class AIS_Manipulator : public AIS_InteractiveObject { public : ! Constructs a manipulator object with default placement and all parts to be displayed. AIS_Manipulator ( ) ; ! Constructs a manipulator object with input location and positions of axes and all parts to be displayed. AIS_Manipulator ( const gp_Ax2 & thePosition ) ; ! Disable or enable visual parts for translation, rotation or scaling for some axis.
...
## ! @warning Raises program error if axis index is < 0 or > 2. void SetPart ( const Standard_Integer theAxisIndex , const AIS_ManipulatorMode theMode , const Standard_Boolean theIsEnabled ) ; ! Disable or enable visual parts for translation, rotation or scaling for ALL axes.
## ! By default all parts are enabled (will be displayed).
## ! @warning Enabling or disabling of visual parts of manipulator does not manage the manipulation (selection) mode.
## ! @warning Raises program error if axis index is < 0 or > 2. void SetPart ( const AIS_ManipulatorMode theMode , const Standard_Boolean theIsEnabled ) ; ! Behavior settings to be applied when performing transformation:
## ! - FollowTranslation - whether the manipulator will be moved together with an object.
## ! - FollowRotation - whether the manipulator will be rotated together with an object. struct OptionsForAttach { OptionsForAttach ( ) : AdjustPosition ( Standard_True ) , AdjustSize ( Standard_False ) , EnableModes ( Standard_True ) { } OptionsForAttach & SetAdjustPosition ( const Standard_Boolean theApply ) { AdjustPosition = theApply ; return * this ; } OptionsForAttach & SetAdjustSize ( const Standard_Boolean theApply ) { AdjustSize = theApply ; return * this ; } OptionsForAttach & SetEnableModes ( const Standard_Boolean theApply ) { EnableModes = theApply ; return * this ; } Standard_Boolean AdjustPosition ; Standard_Boolean AdjustSize ; Standard_Boolean EnableModes ; } ; ! Attaches himself to the input interactive object and become displayed in the same context.
## ! It is placed in the center of object bounding box, and its size is adjusted to the object bounding box. void Attach ( const Handle ( AIS_InteractiveObject ) & theObject , const OptionsForAttach & theOptions = OptionsForAttach ( ) ) ; ! Attaches himself to the input interactive object group and become displayed in the same context.
## ! It become attached to the first object, baut manage manipulation of the whole group.
## ! It is placed in the center of object bounding box, and its size is adjusted to the object bounding box. void Attach ( const Handle ( AIS_ManipulatorObjectSequence ) & theObject , const OptionsForAttach & theOptions = OptionsForAttach ( ) ) ; ! Enable manipualtion mode.
## ! @warning It activates selection mode in the current context.
## ! If manipulator is not displayed, no mode will be activated. void EnableMode ( const AIS_ManipulatorMode theMode ) ; ! Enables mode activation on detection (highlighting).
...
## ! initial data is set automatically, and it is reset on DeactivateCurrentMode call if it is not reset yet. void StartTransform ( const Standard_Integer theX , const Standard_Integer theY , const Handle ( V3d_View ) & theView ) ; ! Apply to the owning objects the input transformation.
...
## ! @warning It will does nothing if transformation is not initiated (with StartTransform() call). void Transform ( const gp_Trsf & aTrsf ) ; ! Reset start (reference) transformation.
## ! @param theToApply [in] option to apply or to cancel the started transformation.
## ! @warning It is used in chain with StartTransform-Transform(gp_Trsf)-StopTransform
## ! and is used only for custom transform set. void StopTransform ( const Standard_Boolean theToApply = Standard_True ) ; ! Apply transformation made from mouse moving from start position
## ! (save on the first Tranform() call and reset on DeactivateCurrentMode() call.)
## ! to the in/out mouse position (theX, theY) gp_Trsf Transform ( const Standard_Integer theX , const Standard_Integer theY , const Handle ( V3d_View ) & theView ) ; ! Computes transformation of parent object according to the active mode and input motion vector.
...
## ! @return transformation of parent object. Standard_Boolean ObjectTransformation ( const Standard_Integer theX , const Standard_Integer theY , const Handle ( V3d_View ) & theView , gp_Trsf & theTrsf ) ; ! Make inactive the current selected manipulator part and reset current axis index and current mode.
## ! After its call HasActiveMode() returns false.
## ! @sa HasActiveMode() void DeactivateCurrentMode ( ) ; ! Detaches himself from the owner object, and removes itself from context. void Detach ( ) ; ! @return all owning objects. Handle ( AIS_ManipulatorObjectSequence ) Objects ( ) const ; ! @return the first (leading) object of the owning objects. Handle ( AIS_InteractiveObject ) Object ( ) const ; ! @return one of the owning objects.
## ! @warning raises program error if theIndex is more than owning objects count or less than 1. Handle ( AIS_InteractiveObject ) Object ( const Standard_Integer theIndex ) const ; ! @return true if manipulator is attached to some interactive object (has owning object). Standard_Boolean IsAttached ( ) const { return HasOwner ( ) ; } ! @return true if some part of manipulator is selected (transformation mode is active, and owning object can be transformed). Standard_Boolean HasActiveMode ( ) const { return IsAttached ( ) && myCurrentMode != AIS_MM_None ; } Standard_Boolean HasActiveTransformation ( ) { return myHasStartedTransformation ; } gp_Trsf StartTransformation ( ) const { return ! myStartTrsfs . IsEmpty ( ) ? myStartTrsfs . First ( ) : gp_Trsf ( ) ; } gp_Trsf StartTransformation ( Standard_Integer theIndex ) const { Standard_ProgramError_Raise_if ( theIndex < 1 || theIndex > Objects ( ) -> Upper ( ) , AIS_Manipulator::StartTransformation(): theIndex is out of bounds ) ; return ! myStartTrsfs . IsEmpty ( ) ? myStartTrsfs ( theIndex ) : gp_Trsf ( ) ; } public : ! @name Configuration of graphical transformations ! Enable or disable zoom persistence mode for the manipulator. With
...
## ! @warning revise use of AdjustSize argument of of \sa AttachToObjects method
## ! when enabling zoom persistence. virtual void SetTransformPersistence ( const Handle ( Graphic3d_TransformPers ) & theTrsfPers ) ; public : ! @name Setters for parameters AIS_ManipulatorMode ActiveMode ( ) const { return myCurrentMode ; } Standard_Integer ActiveAxisIndex ( ) const { return myCurrentIndex ; } ! @return poition of manipulator interactive object. const gp_Ax2 & Position ( ) const { return myPosition ; } ! Sets position of the manipulator object. void SetPosition ( const gp_Ax2 & thePosition ) ; Standard_ShortReal Size ( ) const { return myAxes [ 0 ] . Size ( ) ; } ! Sets size (length of side of the manipulator cubic bounding box. void SetSize ( const Standard_ShortReal theSideLength ) ; ! Sets gaps between translator, scaler and rotator sub-presentations. void SetGap ( const Standard_ShortReal theValue ) ; public : ! Behavior settings to be applied when performing transformation:
## ! - FollowTranslation - whether the manipulator will be moved together with an object.
## ! - FollowRotation - whether the manipulator will be rotated together with an object. struct BehaviorOnTransform { BehaviorOnTransform ( ) : FollowTranslation ( Standard_True ) , FollowRotation ( Standard_True ) , FollowDragging ( Standard_True ) { } BehaviorOnTransform & SetFollowTranslation ( const Standard_Boolean theApply ) { FollowTranslation = theApply ; return * this ; } BehaviorOnTransform & SetFollowRotation ( const Standard_Boolean theApply ) { FollowRotation = theApply ; return * this ; } BehaviorOnTransform & SetFollowDragging ( const Standard_Boolean theApply ) { FollowDragging = theApply ; return * this ; } Standard_Boolean FollowTranslation ; Standard_Boolean FollowRotation ; Standard_Boolean FollowDragging ; } ; ! Sets behavior settings for transformation action carried on the manipulator,
## ! whether it translates, rotates together with the transformed object or not. void SetTransformBehavior ( const BehaviorOnTransform & theSettings ) { myBehaviorOnTransform = theSettings ; } ! @return behavior settings for transformation action of the manipulator. BehaviorOnTransform & ChangeTransformBehavior ( ) { return myBehaviorOnTransform ; } ! @return behavior settings for transformation action of the manipulator. const BehaviorOnTransform & TransformBehavior ( ) const { return myBehaviorOnTransform ; } public : ! @name Presentation computation ! Fills presentation.
## ! @note Manipulator presentation does not use display mode and for all modes has the same presentation. virtual void Compute ( const Handle ( PrsMgr_PresentationManager3d ) & thePrsMgr , const Handle ( Prs3d_Presentation ) & thePrs , const Standard_Integer theMode = 0 ) ; ! Computes selection sensitive zones (triangulation) for manipulator.
## ! @param theNode [in] Selection mode that is treated as transformation mode. virtual void ComputeSelection ( const Handle ( SelectMgr_Selection ) & theSelection , const Standard_Integer theMode ) ; ! Disables auto highlighting to use HilightSelected() and HilightOwnerWithColor() overridden methods. virtual Standard_Boolean IsAutoHilight ( ) const { return Standard_False ; } ! Method which clear all selected owners belonging
## ! to this selectable object ( for fast presentation draw ). virtual void ClearSelected ( ) ; ! Method which draws selected owners ( for fast presentation draw ). virtual void HilightSelected ( const Handle ( PrsMgr_PresentationManager3d ) & thePM , const SelectMgr_SequenceOfOwner & theSeq ) ; ! Method which hilight an owner belonging to
## ! this selectable object  ( for fast presentation draw ). virtual void HilightOwnerWithColor ( const Handle ( PrsMgr_PresentationManager3d ) & thePM , const Handle ( Prs3d_Drawer ) & theStyle , const Handle ( SelectMgr_EntityOwner ) & theOwner ) ; protected : void init ( ) ; void updateTransformation ( ) ; Handle ( Prs3d_Presentation ) getHighlightPresentation ( const Handle ( SelectMgr_EntityOwner ) & theOwner ) const ; Handle ( Graphic3d_Group ) getGroup ( const Standard_Integer theIndex , const AIS_ManipulatorMode theMode ) const ; void attachToBox ( const Bnd_Box & theBox ) ; void adjustSize ( const Bnd_Box & theBox ) ; void setTransformPersistence ( const Handle ( Graphic3d_TransformPers ) & theTrsfPers ) ; ! Redefines local transformation management method to inform user of inproper use.
