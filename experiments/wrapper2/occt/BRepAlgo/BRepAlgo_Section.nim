...
## ! on edges built only on new intersection lines. void Approximation ( const Standard_Boolean B ) ; ! Indicates if the Pcurve must be (or not) performed on first part. void ComputePCurveOn1 ( const Standard_Boolean B ) ; ! Define options for the computation of further
...
## ! edges built on new intersection lines. void ComputePCurveOn2 ( const Standard_Boolean B ) ; ! Performs the computation of the section lines
...
## ! the use of the constructors. void Build ( ) ; ! Identifies the ancestor faces of the new
...
## ! Boolean value equals true. Standard_Boolean HasAncestorFaceOn1 ( const TopoDS_Shape & E , TopoDS_Shape & F ) const ; ! Identifies the ancestor faces of the new
## ! intersection edge E resulting from the last
## ! computation performed in this framework, that is,
## ! the faces of the two original shapes on which the edge E lies:
