...
## !!!Ignored construct:  # _BVH_BinnedBuilder_Header [NewLine] # _BVH_BinnedBuilder_Header [NewLine] # < BVH_QueueBuilder . hxx > [NewLine] # < algorithm > [NewLine] # defined ( _WIN32 ) && defined ( max ) [NewLine] # max [NewLine] # [NewLine] # < limits > [NewLine] ! Stores parameters of single bin (slice of AABB). template < class T , int N > [end of template] struct BVH_Bin { ! Creates new node bin. BVH_Bin ( ) : Count ( 0 ) { } Standard_Integer Count ; !< Number of primitives in the bin BVH_Box < T , N > [end of template] Box ; !< AABB of primitives in the bin } ;
...
## !!!Ignored construct:  template < class T , int N , int Bins = BVH_Constants_NbBinsOptimal > [end of template] class BVH_BinnedBuilder : public BVH_QueueBuilder < T , N > { public : ! Type of the array of bins of BVH tree node. typedef BVH_Bin < T , N > BVH_BinVector [ Bins ] ; ! Describes split plane candidate. struct BVH_SplitPlane { BVH_Bin < T , N > LftVoxel ; BVH_Bin < T , N > RghVoxel ; } ; ! Type of the array of split plane candidates. typedef BVH_SplitPlane BVH_SplitPlanes [ Bins + 1 ] ; public : ! Creates binned SAH BVH builder. BVH_BinnedBuilder ( const Standard_Integer theLeafNodeSize = BVH_Constants_LeafNodeSizeDefault , const Standard_Integer theMaxTreeDepth = BVH_Constants_MaxTreeDepth , const Standard_Boolean theDoMainSplits = Standard_False , const Standard_Integer theNumOfThreads = 1 ) : BVH_QueueBuilder < T , N > ( theLeafNodeSize , theMaxTreeDepth , theNumOfThreads ) , myUseMainAxis ( theDoMainSplits ) {  } ! Releases resources of binned SAH BVH builder. virtual ~ BVH_BinnedBuilder ( ) { } protected : ! Performs splitting of the given BVH node. virtual typename BVH_QueueBuilder < T , N > :: BVH_ChildNodes buildNode ( BVH_Set < T , N > * theSet , BVH_Tree < T , N > * theBVH , const Standard_Integer theNode ) const ; ! Arranges node primitives into bins. virtual void getSubVolumes ( BVH_Set < T , N > * theSet , BVH_Tree < T , N > * theBVH , const Standard_Integer theNode , BVH_BinVector & theBins , const Standard_Integer theAxis ) const ; private : Standard_Boolean myUseMainAxis ; !< Defines whether to search for the best split or use the widest axis } ;
## Error: token expected: > [end of template] but got: =!!!
