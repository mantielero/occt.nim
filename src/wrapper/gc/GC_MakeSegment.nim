...
proc constructor_GC_MakeSegment*(Line: gp_Lin, U1: Standard_Real, U2: Standard_Real): GC_MakeSegment {.constructor,importcpp: "GC_MakeSegment(@)".}
  ## Make a segment of Line from the line <Line1> between the two
  ## parameters U1 and U2. It returns NullObject if <U1> is equal <U2>.

proc constructor_GC_MakeSegment*(Line: gp_Lin, Point: gp_Pnt, Ulast: Standard_Real): GC_MakeSegment {.constructor,importcpp: "GC_MakeSegment(@)".}
