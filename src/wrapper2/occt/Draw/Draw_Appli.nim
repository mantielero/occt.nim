...
when not defined windows:
  ...
  var drawBatch* {.importcpp: "Draw_Batch", header: "Draw_Appli.hxx".}: StandardBoolean
type
  DrawSaveAndRestore* {.importcpp: "Draw_SaveAndRestore", header: "Draw_Appli.hxx",
                       bycopy.} = object


proc constructDrawSaveAndRestore*(name: cstring; test: proc (
    a1: Handle[DrawDrawable3D]): StandardBoolean; save: proc (
    a1: Handle[DrawDrawable3D]; a2: var Ostream); restore: proc (a1: var Istream): Handle[
    DrawDrawable3D]; display: StandardBoolean = standardTrue): DrawSaveAndRestore {.
...
proc test*(this: var DrawSaveAndRestore; d: Handle[DrawDrawable3D]): StandardBoolean {.
...
proc disp*(this: DrawSaveAndRestore): StandardBoolean {.noSideEffect,
