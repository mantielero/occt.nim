...
type
  Graphic3dRenderingParams* {.importcpp: "Graphic3d_RenderingParams",
    ...
    pbrEnvPow2Size* {.importc: "PbrEnvPow2Size".}: StandardInteger ## !< size of IBL maps side can be calculated as 2^PbrEnvPow2Size (> 0), 9 by default
    pbrEnvSpecMapNbLevels* {.importc: "PbrEnvSpecMapNbLevels".}: StandardInteger ## !< number of levels used in specular IBL map (> 1), 6 by default
    pbrEnvBakingDiffNbSamples* {.importc: "PbrEnvBakingDiffNbSamples".}: StandardInteger ## !< number of samples used in Monte-Carlo integration during diffuse IBL map's
                                                                                     ## !  spherical harmonics coefficients generation (> 0), 1024 by default
    pbrEnvBakingSpecNbSamples* {.importc: "PbrEnvBakingSpecNbSamples".}: StandardInteger ## !< number of samples used in Monte-Carlo integration during specular IBL map's generation (> 0), 256 by default
    ...
    nbMsaaSamples* {.importc: "NbMsaaSamples".}: StandardInteger ## !< number of MSAA samples (should be within 0..GL_MAX_SAMPLES, power-of-two number), 0 by default
    ...
    toEnableDepthPrepass* {.importc: "ToEnableDepthPrepass".}: StandardBoolean ## !< enables/disables depth pre-pass, False by default
    toEnableAlphaToCoverage* {.importc: "ToEnableAlphaToCoverage".}: StandardBoolean ## !< enables/disables alpha to coverage, True by default
    isGlobalIlluminationEnabled* {.importc: "IsGlobalIlluminationEnabled".}: StandardBoolean ## !< enables/disables global illumination effects (path tracing)
    samplesPerPixel* {.importc: "SamplesPerPixel".}: StandardInteger ## !< number of samples per pixel (SPP)
    raytracingDepth* {.importc: "RaytracingDepth".}: StandardInteger ## !< maximum ray-tracing depth, 3 by default
    isShadowEnabled* {.importc: "IsShadowEnabled".}: StandardBoolean ## !< enables/disables shadows rendering, True by default
    isReflectionEnabled* {.importc: "IsReflectionEnabled".}: StandardBoolean ## !< enables/disables specular reflections, False by default
    isAntialiasingEnabled* {.importc: "IsAntialiasingEnabled".}: StandardBoolean ## !< enables/disables adaptive anti-aliasing, False by default
    isTransparentShadowEnabled* {.importc: "IsTransparentShadowEnabled".}: StandardBoolean ## !< enables/disables light propagation through transparent media, False by default
    useEnvironmentMapBackground* {.importc: "UseEnvironmentMapBackground".}: StandardBoolean ## !< enables/disables environment map background
    toIgnoreNormalMapInRayTracing* {.importc: "ToIgnoreNormalMapInRayTracing".}: StandardBoolean ## !< enables/disables normal map ignoring during path tracing; FALSE by default
    coherentPathTracingMode* {.importc: "CoherentPathTracingMode".}: StandardBoolean ## !< enables/disables 'coherent' tracing mode (single RNG seed within 16x16 image blocks)
    adaptiveScreenSampling* {.importc: "AdaptiveScreenSampling".}: StandardBoolean ## !< enables/disables adaptive screen sampling mode for path tracing, FALSE by default
    adaptiveScreenSamplingAtomic* {.importc: "AdaptiveScreenSamplingAtomic".}: StandardBoolean ## !< enables/disables usage of atomic float operations within adaptive screen sampling, FALSE by default
    showSamplingTiles* {.importc: "ShowSamplingTiles".}: StandardBoolean ## !< enables/disables debug mode for adaptive screen sampling, FALSE by default
    twoSidedBsdfModels* {.importc: "TwoSidedBsdfModels".}: StandardBoolean ## !< forces path tracing to use two-sided versions of original one-sided scattering models
    ...
    rebuildRayTracingShaders* {.importc: "RebuildRayTracingShaders".}: StandardBoolean ## !< forces rebuilding ray tracing shaders at the next frame
    rayTracingTileSize* {.importc: "RayTracingTileSize".}: StandardInteger ## !< screen tile size, 32 by default (adaptive sampling mode of path tracing);
    nbRayTracingTiles* {.importc: "NbRayTracingTiles".}: StandardInteger ## !< maximum number of screen tiles per frame, 256 by default (adaptive sampling mode of path tracing);
    ...
    toReverseStereo* {.importc: "ToReverseStereo".}: StandardBoolean ## !< flag to reverse stereo pair, FALSE by default
    toMirrorComposer* {.importc: "ToMirrorComposer".}: StandardBoolean ## !< if output device is an external composer - mirror rendering results in window in addition to sending frame to composer, TRUE by default
    ...
    statsTextHeight* {.importc: "StatsTextHeight".}: StandardInteger ## !< stats text size; 16 by default
    statsNbFrames* {.importc: "StatsNbFrames".}: StandardInteger ## !< number of data frames to collect history; 1 by default
    ...
    toShowStats* {.importc: "ToShowStats".}: StandardBoolean ## !< display performance statistics, FALSE by default;
