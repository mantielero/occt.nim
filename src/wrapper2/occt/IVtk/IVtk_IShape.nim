...
## ! @brief Interface for working with a shape and its sub-shapes ids. class IVtk_IShape : public IVtk_Interface { public : typedef Handle ( IVtk_IShape ) Handle ; virtual ~ IVtk_IShape ( ) { } IVtk_IdType GetId ( ) const { return myId ; } void SetId ( const IVtk_IdType theId ) { myId = theId ; } ! Get ids of sub-shapes composing a sub-shape with the given id virtual IVtk_ShapeIdList GetSubIds ( const IVtk_IdType theId ) const = 0 ; DEFINE_STANDARD_RTTIEXT ( IVtk_IShape , IVtk_Interface ) private : IVtk_IdType myId ; } ;
## Error: expected ';'!!!
