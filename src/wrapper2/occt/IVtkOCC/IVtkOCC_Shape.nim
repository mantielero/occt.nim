...
## ! @return local ID of the sub-shape. IVtk_IdType GetSubShapeId ( const TopoDS_Shape & theSubShape ) const ; ! Get ids of sub-shapes composing a sub-shape with the given id IVtk_ShapeIdList GetSubIds ( const IVtk_IdType ) const Standard_OVERRIDE ; ! @brief Get a sub-shape by its local ID.
## !
