...
type
  IVtkPolyline* = NCollectionSequence[GpPnt]
...
## ! @return deviation angle (in radians) Standard_Real GetDeviationAngle ( ) const { return myDevAngle ; } protected : ! Executes the mesh generation algorithms. To be defined in implementation class. virtual void internalBuild ( ) Standard_OVERRIDE ; private : ! Internal method, generates OCCT triangulation starting from TopoDS_Shape
...
## ! The polyline is associated with the given sub-shape ID. void processPolyline ( Standard_Integer theNbNodes , const TColgp_Array1OfPnt & thePoints , const TColStd_Array1OfInteger & thePointIds , const IVtk_IdType theOcctId , bool theNoTransform , gp_Trsf theTransformation , const IVtk_MeshType theMeshType ) ; ! Get the IShape as OCC implementation const IVtkOCC_Shape :: Handle GetShapeObj ( ) const ; DEFINE_STANDARD_RTTIEXT ( IVtkOCC_ShapeMesher , IVtk_IShapeMesher ) private : IVtk_ShapeTypeMap myEdgesTypes ; Standard_Real myDevCoeff ; Standard_Real myDevAngle ; mutable Standard_Real myDeflection ; Standard_Integer myNbIsos [ 2 ] ; } ;
## Error: expected ';'!!!
