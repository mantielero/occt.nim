...
## !!!Ignored construct:  template < class Inspector > [end of template] class NCollection_CellFilter { public : typedef TYPENAME Inspector :: Target Target ; typedef TYPENAME Inspector :: Point Point ; public : ! Constructor; initialized by dimension count and cell size.
## !
...
## ! Constructor when dimension count is unknown at compilation time. NCollection_CellFilter ( const Standard_Integer theDim , const Standard_Real theCellSize = 0 , const opencascade :: handle < NCollection_IncAllocator > [end of template] & theAlloc = 0 ) : myCellSize ( 0 , theDim - 1 ) { myDim = theDim ; Reset ( theCellSize , theAlloc ) ; } ! Constructor when dimenstion count is known at compilation time. NCollection_CellFilter ( const Standard_Real theCellSize = 0 , const opencascade :: handle < NCollection_IncAllocator > [end of template] & theAlloc = 0 ) : myCellSize ( 0 , Inspector :: Dimension - 1 ) { myDim = Inspector :: Dimension ; Reset ( theCellSize , theAlloc ) ; } ! Clear the data structures, set new cell size and allocator void Reset ( Standard_Real theCellSize , const opencascade :: handle < NCollection_IncAllocator > [end of template] & theAlloc = 0 ) { for ( int i = 0 ; i < myDim ; i ++ ) myCellSize ( i ) = theCellSize ; resetAllocator ( theAlloc ) ; } ! Clear the data structures and set new cell sizes and allocator void Reset ( NCollection_Array1 < Standard_Real > [end of template] & theCellSize , const opencascade :: handle < NCollection_IncAllocator > [end of template] & theAlloc = 0 ) { myCellSize = theCellSize ; resetAllocator ( theAlloc ) ; } ! Adds a target object for further search at a point (into only one cell) void Add ( const Target & theTarget , const Point & thePnt ) { Cell aCell ( thePnt , myCellSize ) ; add ( aCell , theTarget ) ; } ! Adds a target object for further search in the range of cells
## ! defined by two points (the first point must have all co-ordinates equal or
## ! less than the same co-ordinate of the second point) void Add ( const Target & theTarget , const Point & thePntMin , const Point & thePntMax ) {  get cells range by minimal and maximal co-ordinates Cell aCellMin ( thePntMin , myCellSize ) ; Cell aCellMax ( thePntMax , myCellSize ) ; Cell aCell = aCellMin ;  add object recursively into all cells in range iterateAdd ( myDim - 1 , aCell , aCellMin , aCellMax , theTarget ) ; } ! Find a target object at a point and remove it from the structures.
## ! For usage of this method "operator ==" should be defined for Target. void Remove ( const Target & theTarget , const Point & thePnt ) { Cell aCell ( thePnt , myCellSize ) ; remove ( aCell , theTarget ) ; } ! Find a target object in the range of cells defined by two points and
...
## ! For usage of this method "operator ==" should be defined for Target. void Remove ( const Target & theTarget , const Point & thePntMin , const Point & thePntMax ) {  get cells range by minimal and maximal co-ordinates Cell aCellMin ( thePntMin , myCellSize ) ; Cell aCellMax ( thePntMax , myCellSize ) ; Cell aCell = aCellMin ;  remove object recursively from all cells in range iterateRemove ( myDim - 1 , aCell , aCellMin , aCellMax , theTarget ) ; } ! Inspect all targets in the cell corresponding to the given point void Inspect ( const Point & thePnt , Inspector & theInspector ) { Cell aCell ( thePnt , myCellSize ) ; inspect ( aCell , theInspector ) ; } ! Inspect all targets in the cells range limited by two given points
## ! (the first point must have all co-ordinates equal or
## ! less than the same co-ordinate of the second point) void Inspect ( const Point & thePntMin , const Point & thePntMax , Inspector & theInspector ) {  get cells range by minimal and maximal co-ordinates Cell aCellMin ( thePntMin , myCellSize ) ; Cell aCellMax ( thePntMax , myCellSize ) ; Cell aCell = aCellMin ;  inspect object recursively into all cells in range iterateInspect ( myDim - 1 , aCell , aCellMin , aCellMax , theInspector ) ; } # defined ( __SUNPRO_CC ) && ( __SUNPRO_CC <= 0x530 ) [NewLine] public :  work-around against obsolete SUN WorkShop 5.3 compiler # [NewLine] protected : # [NewLine] *
##  Auxiliary class for storing points belonging to the cell as the list
##  struct ListNode { ListNode ( ) {  Empty constructor is forbidden. throw Standard_NoSuchObject ( NCollection_CellFilter::ListNode() ) ; } Target Object ; ListNode * Next ; } ; *
...
##  struct Cell { public : ! Constructor; computes cell indices Cell ( const Point & thePnt , const NCollection_Array1 < Standard_Real > [end of template] & theCellSize ) : index ( theCellSize . Size ( ) ) , Objects ( 0 ) { for ( int i = 0 ; i < theCellSize . Size ( ) ; i ++ ) { Standard_Real val = ( Standard_Real ) ( Inspector :: Coord ( i , thePnt ) / theCellSize ( theCellSize . Lower ( ) + i ) ) ; If the value of index is greater than
...
proc coord*(i: cint; thePnt: NCollectionCellFilterNCollectionCellFilterNCollectionCellFilterInspectorXYZPoint): StandardReal {.
...
proc shift*(this: NCollectionCellFilterNCollectionCellFilterNCollectionCellFilterInspectorXYZ;
    thePnt: NCollectionCellFilterNCollectionCellFilterNCollectionCellFilterInspectorXYZPoint;
           theTol: StandardReal): NCollectionCellFilterNCollectionCellFilterNCollectionCellFilterInspectorXYZPoint {.
...
proc coord*(i: cint; thePnt: NCollectionCellFilterNCollectionCellFilterNCollectionCellFilterInspectorXYPoint): StandardReal {.
...
proc shift*(this: NCollectionCellFilterNCollectionCellFilterNCollectionCellFilterInspectorXY;
    thePnt: NCollectionCellFilterNCollectionCellFilterNCollectionCellFilterInspectorXYPoint;
           theTol: StandardReal): NCollectionCellFilterNCollectionCellFilterNCollectionCellFilterInspectorXYPoint {.
    noSideEffect, importcpp: "Shift", header: "NCollection_CellFilter.hxx".}
