...
type
  Quantity_TypeOfColor* {.size: sizeof(cint), importcpp: "Quantity_TypeOfColor",
                         header: "Quantity_TypeOfColor.hxx".} = enum ## ! Normalized linear RGB (red, green, blue) values within range [0..1] for each component
    Quantity_TOC_RGB,         ## ! Normalized non-linear gamma-shifted RGB (red, green, blue) values within range [0..1] for each component
    Quantity_TOC_sRGB, ## ! Hue + light + saturation components, where:
    ...
    Quantity_TOC_HLS, ## ! CIE L*a*b* color space, constructed to be perceptually uniform for human eye.
    ...
    Quantity_TOC_CIELab, ## ! CIE L*c*h* color space, same as L*a*b* in cylindrical coordinates:
                        ## ! - L: lightness in range [0, 100] (from black to white)
